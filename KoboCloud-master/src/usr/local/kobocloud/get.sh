#!/bin/sh
#Kobocloud getter - Firebase fork
...
while read url || [ -n "$url" ]; do
  ...
  # ── Firebase (new) ──────────────────────────────────────────────────────────
  elif echo "$url" | grep -q '^Firebase:'; then
    rest=`echo "$url" | sed 's/^Firebase://'`
    DEVICE_ID=`echo "$rest" | sed 's/:.*//'`
    DEVICE_TOKEN=`echo "$rest" | sed 's/[^:]*://'`
    echo "Firebase sync for device $DEVICE_ID"
    $KC_HOME/getFirebaseFiles.sh "$DEVICE_ID" "$DEVICE_TOKEN"
  # ───────────────────────────────────────────────────────────────────────────
  else
    echo "Getting $url"
